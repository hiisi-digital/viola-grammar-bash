...
Deno.test("strings - double-quoted string", async () => {
  ...
  assertEquals(
    ...
    true,
    "Should find at least one string",
  );
...
`);
  assertEquals(
    data.strings.length >= 1,
    true,
    "Should find at least one string from here-doc",
  );
});
...
`);
  ...
  assertEquals(
    ...
    true,
    "Should find exported function name",
  );
});
...
`);
  ...
  assertEquals(
    fn.isExported,
    false,
    "Function should NOT be marked as exported",
  );
});
...
Deno.test("queries - functions query compiles", async () => {
  const captures = await queryCaptures(
    bash.queries.functions,
    `
foo() { echo "test"; }
`,
  );
  assertEquals(
    captures.length >= 1,
    true,
    "Function query should produce captures",
  );
});

Deno.test("queries - exports query compiles", async () => {
  const captures = await queryCaptures(
    bash.queries.exports!,
    `
export MY_VAR="test"
`,
  );
  assertEquals(
    captures.length >= 1,
    true,
    "Export query should produce captures",
  );
});

Deno.test("queries - imports query compiles", async () => {
  const captures = await queryCaptures(
    bash.queries.imports!,
    `
source ./test.sh
`,
  );
  assertEquals(
    captures.length >= 1,
    true,
    "Import query should produce captures",
  );
});

Deno.test("queries - strings query compiles", async () => {
  const captures = await queryCaptures(
    bash.queries.strings!,
    `
echo "hello"
`,
  );
  assertEquals(
    captures.length >= 1,
    true,
    "String query should produce captures",
  );
});
...
Deno.test("integration - realistic bash script extraction", async () => {
  ...
  assertEquals(
    funcNames.includes("build"),
    true,
    "Should extract build function",
  );
  assertEquals(
    funcNames.includes("deploy"),
    true,
    "Should extract deploy function",
  );
  assertEquals(
    funcNames.includes("run_tests"),
    true,
    "Should extract run_tests function",
  );
  ...
  assertEquals(
    buildFn.params.length >= 1,
    true,
    "build should have at least 1 param",
  );
  ...
  assertEquals(
    deployFn.params.length >= 2,
    true,
    "deploy should have at least 2 params",
  );
